...
public class ComponentButtonManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int i = 0; i<favoriteComponents.Count && i<itemLimit;i++)
        {
            ...
            newButt.transform.position = new Vector3(this.transform.position.x - (this.GetComponent<RectTransform>().sizeDelta.x/2) + (i * spacing), this.transform.transform.position.y, 0f);            newButt.linkedComponent = favoriteComponents[i];
